...
private:
    ...
    int add_net_aduit(std::string exe_path,std::string trigger,std::string action,pkt_limit_v4 *limit);
    ...
    int set_scale(const nlohmann::json &json);
    void set_process(const nlohmann::json &json,std::vector<int> &pids,std::vector<std::string> &exe_paths);
    void set_uids(const nlohmann::json &json,std::vector<int> &uids);
    void set_gids(const nlohmann::json &json,std::vector<int> &gids);
    
    int set_protocol(const nlohmann::json &json);
  ...
  //  void update_rules();
    void clear_rules();
    void clear_map(int map_fd);
public:
    void init(struct bpf_object *obj);
    void parse();
    void updata_rules();
    void wait_command(std::string command);
};