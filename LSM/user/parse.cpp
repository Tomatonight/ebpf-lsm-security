...
static std::vector<std::pair<std::string, int>> Triggers =
    {{"open", TRIGGER_OPEN}, {"open_write", TRIGGER_OPEN_WRITE}, {"open_read", TRIGGER_OPEN_READ}, {"open_rw", TRIGGER_OPEN_RW}, {"link", TRIGGER_LINK}, {"unlink", TRIGGER_UNLINK}, {"truncate", TRIGGER_TRUNCATE}, {"create_file", TRIGGER_CREATE_FILE}, {"mkdir", TRIGGER_MKDIR}, {"rmdir", TRIGGER_RMDIR}, {"create_thread", TRIGGER_CREATE_THREAD}, {"fork", TRIGGER_FORK}, {"execve", TRIGGER_EXECVE}, {"ptrace", TRIGGER_PTRACE}, {"ptraceme", TRIGGER_PTRACEME}, {"socket_create", TRIGGER_CREATE_SOCKET}, {"listen", TRIGGER_LISTEN}, {"send_msg", TRIGGER_SEND_MSG}, {"recv_msg", TRIGGER_RECV_MSG}, {"bind", TRIGGER_BIND}, {"connect", TRIGGER_CONNECT}, {"accept", TRIGGER_CONNECT}, {"accept", TRIGGER_ACCEPT}, {"shutdown", TRIGGER_SHUTDOWN}};
//
...
struct operator_index
{
  ...
  __u32 uids[OPERATOR_MAX];
  int uid_nb;
  __u32 gids[OPERATOR_MAX];
  int gid_nb;
  __u32 pids[OPERATOR_MAX];
  int pid_nb;
  __u32 exe_inodes[OPERATOR_MAX];
  int exe_inode_nb;
};
struct file_audit
{
...
void json_parse::init(struct bpf_object *obj)
{
  ...
  char buffer[2048] = {0};
  int file_fd = open("configure/configure.json", O_RDWR);
  if (file_fd < 0)
  {
...
void json_parse::parse()
{
  if (data.contains("file") && data["file"].is_array())
  {
    for (const auto &fileEntry : data["file"])
    {
      ...
      bool is_exclude_or_include;
      if (limited_task.contains("exclude"))
      {
        is_exclude_or_include = true;
      }
      else if (limited_task.contains("include"))
      {
        is_exclude_or_include = false;
      }
      else
      {
        printf("visit_index err\n");
        continue;
      }
      /*
      "uid":["root","yueyiqiao"],
      ...
      for (int i = 0; i < limited_task["uid"].size(); i++)
      {
        if (limited_task["uid"][i].is_string())
        {
          std::string username = limited_task["uid"][i];
          int uid;
          if (get_uid((char *)username.data(), &uid) < 0)
          {
            continue;
          }
          uids.push_back(uid);
        }
        else if (limited_task["uid"][i].is_number_integer())
        {
          int uid = limited_task["uid"][i];
          uids.push_back(uid);
        }
      }
      for (int i = 0; i < limited_task["gid"].size(); i++)
      {
        if (limited_task["gid"][i].is_string())
        {
          std::string groupname = limited_task["gid"][i];
          int gid;
          if (get_gid((char *)groupname.data(), &gid) < 0)
          {
            continue;
          }
          gids.push_back(gid);
        }
        else if (limited_task["gid"][i].is_number_integer())
        {
          int gid = limited_task["gid"][i];
          gids.push_back(gid);
        }
      }
      for (int i = 0; i < limited_task["process"].size(); i++)
      {
        if (limited_task["process"][i].is_string())
        {
          exe_paths.push_back(limited_task["process"][i]);
        }
        else if (limited_task["process"][i].is_number_integer())
        {
          pids.push_back(limited_task["process"][i]);
        }
      }
      if (uids.size() > OPERATOR_MAX || gids.size() > OPERATOR_MAX || pids.size() > OPERATOR_MAX || exe_paths.size() > OPERATOR_MAX)
      {
        printf("size >%d\n", OPERATOR_MAX);
        continue;
      }
      ...
      for (const std::string &path : fileEntry["path"])
      {
        ...
        {
          add_file_audit(path, trigger, action, is_exclude_or_include, exe_paths, pids, gids, uids);
        }
      }
    }
  }
  ...
  if (data.contains("process") && data["process"].is_array())
  {
    for (const auto &process : data["process"])
    {
      ...
      for (int i = 0; i < process["process"].size(); i++)
      {
        if (process["process"][i].is_string())
        {
          std::string path = process["process"][i];
          for (const std::string &trigger : process["trigger"])
          {
            add_process_audit(path, trigger, action);
          }
        }
        else if (process["process"][i].is_number_integer())
        {
          int pid = process["process"][i];
          for (const std::string &trigger : process["trigger"])
          {
            for (const std::string &trigger : process["trigger"])
            {
  ...
  if (data.contains("network") && data["network"].is_array())
  {
    ...
    for (const auto &net : data["network"])
    {
      ...
      if (!net.contains("visit_index"))
      {
        printf("net visit index lose\n");
        continue;
      }

      const auto &visit = net["visit_index"];
      if (visit.contains("exclude"))
      {
        limit.exclude_or_include = true;
      }
      else if (visit.contains("include"))
      {
        limit.exclude_or_include = false;
      }
      else
      {
        printf("net visit index err\n");
        continue;
      }
      ...
      for (int i = 0; i < net["process"].size(); i++)
      {
        if (net["process"][i].is_number_integer())
        {
          int pid = net["process"][i];
          for (const std::string &trigger : net["trigger"])
          {
            add_net_aduit(pid, trigger, action, &limit);
          }
        }
        else if (net["process"][i].is_string())
        {
          std::string path = net["process"][i];
          for (const std::string &trigger : net["trigger"])
          {
...
int json_parse::add_file_audit(std::string path_name, std::string trigger, std::string action, bool exclude_or_include, std::vector<std::string> &exe_paths, std::vector<int> &pids, std::vector<int> &gids, std::vector<int> &uids)
{
  ...
  for (auto &it : Triggers)
  {
    if (it.first == trigger)
    {
      new_file_audit.trigger = it.second;
      break;
    }
  }
  if (!new_file_audit.trigger)
  {
    printf("trigger err\n");
    return -1;
  }
  for (auto &it : Actions)
  {
    if (it.first == action)
    {
      ...
      new_file_audit.action = it.second;
      break;
    }
  }
  if (!new_file_audit.action)
  {
    printf("action err %s\n", action.data());
    return -1;
  }
...
int json_parse::add_net_aduit(int exe_inode, std::string trigger, std::string action, pkt_limit_v4 *limit)
{
  // if (!exist_file(exe_path))
  // {
  //   printf("path err %s\n", exe_path.data());
...
void json_parse::set_process(const nlohmann::json &json, std::vector<int> &pids, std::vector<int> &exe_inodes)
{
  for (const auto &item : json)
  {
    ...
    else if (item.is_string())
    {
      ...
      if (exist_file(path))
      {
        int inode = get_file_inode(path);
        if (inode == -1)
        {
          printf("file path:%s err\n", path.data());
          continue;
        }
        exe_inodes.push_back(inode);
      }
    }
  }
}
